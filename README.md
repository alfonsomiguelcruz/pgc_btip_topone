...
After organizing the sequences, all sequences in each file were aligned using the [Nextclade CLI](https://clades.nextstrain.org/) against the [SARS-CoV-2 Wuhan reference genome](https://github.com/nextstrain/nextclade_data/blob/release/data/nextstrain/sars-cov-2/wuhan-hu-1/orfs/reference.fasta). The aligned sequences were processed further by removing a column of nucleotides in all sequences if at least one sequence contains an unknown nucleotide. This processing results in sequences containing the four nucleotides (`A`, `T`, `C`, `G`) and gaps (`-`) only.
